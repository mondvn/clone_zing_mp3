...
const { RiVipCrown2Line, BsFillPlayFill, SlHeart, AiFillHeart } = icons

const ArtistSongitem = ({ song, isAlbum, playlistSong }) => {
  ...
  const playlist = async () => {
    const data = {
      title: '',
      link: '',
      songs: playlistSong.filter(item => item.isWorldWide)
    }
    dispatch(actions.setCurPlaylist(data))
  }

  const playlistWithShuffle = async () => {
    const arr = playlistSong
    const currentSongIndex = playlistSong?.findIndex(item => item.encodeId === song?.encodeId)
...
export default memo(ArtistSongitem)